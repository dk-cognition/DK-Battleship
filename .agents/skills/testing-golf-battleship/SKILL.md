...
dotnet run --project src/DKBattleship.Web --urls http://0.0.0.0:5099
dotnet test tests/DKBattleship.Tests             # unit tests
```
...
Clicking 100 cells by hand is slow. A console interval that clicks fresh cells works well (the UI has a ~650 ms AI delay, so use >= 600 ms):
```js
window.__driver=setInterval(()=>{
  const grids=document.querySelectorAll('.grid');
  if(grids.length<2){clearInterval(window.__driver);return;}          // game over
...
- Verify placement counts with `document.querySelectorAll('.grid')[1].querySelectorAll('.ship').length === 17` (Driver 5 + Fairway Wood 4 + Hybrid 3 + Iron 3 + Putter 2) — a count of 17 also proves no overlaps.
